...
const MathRenderer: React.FC<MathRendererProps> = ({ text, className = "", isChat = false }) => {
    ...
    return (
        <div className={`precision-content-block break-words overflow-hidden ${className} ${isChat ? 'text-sm md:text-base' : ''}`}>
            <ReactMarkdown
                remarkPlugins={[remarkGfm]}
                components={{
                    p: ({ children }) => <p className={`${isChat ? 'mb-2' : 'mb-4'} leading-relaxed font-medium`}>{children}</p>,
                    // NEON HIGHLIGHTING FOR OMEGA THEME
                    strong: ({ children }) => <strong className="font-black text-cyan-400 drop-shadow-[0_0_12px_rgba(34,211,238,0.5)] italic">{children}</strong>,
                    em: ({ children }) => <em className="text-violet-400 font-black not-italic drop-shadow-[0_0_8px_rgba(167,139,250,0.4)] italic">{children}</em>,
                    h1: ({ children }) => <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tighter text-white mb-6 border-l-8 border-violet-600 pl-6 bg-white/5 py-3 rounded-r-3xl">{children}</h1>,
