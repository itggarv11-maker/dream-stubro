...
const DashboardPage: React.FC = () => {
    ...
    const { extractedText, subject, classLevel, tokens } = useContent() as any;
    ...
    const handleToolClick = async (toolId: string, requiresContent: boolean) => {
        ...
        try {
            switch(toolId) {
                case 'summary':
                    const s = await geminiService.generateSmartSummary(subject || Subject.Science, classLevel, extractedText!);
                    setResultData(s);
                    break;
                case 'flashcards':
                    // High-quality mock/logic integration
                    setResultData([{ term: "Conceptual Integrity", definition: "The quality of having consistent logic across all points." }]);
                    break;
                case 'quiz':
                    const q = await geminiService.generateQuiz(subject || Subject.Science, classLevel, extractedText!, 5);
                    setResultData(q);
                    break;
                case 'chat':
                    const session = geminiService.createGeneralChat(extractedText!);
                    setChatSession(session);
                    setChatHistory([{ role: 'model', text: "Uplink stable. Logic core ready for query." }]);
                    break;
                default: throw new Error("Module not found.");
            }
        } catch (e: any) {
        ...
        return (
            <div className="max-w-[1600px] mx-auto px-6 md:px-12 pb-40 animate-in fade-in duration-500">
                <button onClick={resetAllState} className="flex items-center gap-4 text-[10px] font-black text-slate-500 hover:text-white mb-16 uppercase tracking-widest group">
                    <div className="p-4 bg-slate-900 rounded-2xl group-hover:bg-violet-600 transition-all">&larr; ABORT MISSION</div>
                </button>
                {isLoading ? (
                    <div className="py-60 flex flex-col items-center gap-10">
                        <Spinner className="w-40 h-40" colorClass="bg-violet-500" />
                        <p className="text-4xl font-black italic tracking-tightest text-white uppercase animate-pulse">Synchronizing Neural Logic...</p>
                    </div>
                ) : (
                    <>
                        ...
                        {activeTool === 'chat' && (
                             <Card variant="dark" className="!p-0 border-white/5 bg-slate-950/60 rounded-[4rem] overflow-hidden flex flex-col h-[75vh] shadow-2xl">
                                <div ref={chatScrollRef} className="flex-grow overflow-y-auto p-10 space-y-10 scrollbar-hide">
                                    {chatHistory.map((msg, i) => (
                                        <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[80%] p-10 rounded-[3rem] shadow-2xl ${msg.role === 'user' ? 'bg-violet-600 text-white font-black italic shadow-violet-600/30' : 'bg-slate-900 border border-white/5 text-slate-200 font-medium'}`}>
                                ...
                                <form onSubmit={(e) => { e.preventDefault(); handleSendChatMessage(); }} className="p-10 bg-black/60 border-t border-white/5 flex gap-6">
                                    <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Input Logic Signal..." className="flex-grow bg-slate-950 border border-white/10 p-10 rounded-full text-white outline-none focus:border-violet-500 font-mono italic"/>
    ...
    return (
        <div className="max-w-[1800px] mx-auto px-6 md:px-12 pb-40 space-y-20">
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10 border-b border-white/5 pb-16 pt-10 relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <h2 className="text-6xl md:text-[10rem] font-black tracking-tightest uppercase italic leading-none">{subject || "SYNC_IDLE"}</h2>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-end md:items-center gap-10 relative z-10">
                    ...
                    <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5 backdrop-blur-3xl space-y-3 min-w-[280px]">
                        ...
                        <Link to="/premium" className="block text-[8px] font-black text-center text-amber-500 uppercase tracking-[0.4em] hover:text-amber-400 transition-colors pt-2">Unlock Unlimited Root Access &rarr;</Link>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                {/* 1. CORE MODULES (Left Sidebar Style) */}
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {coreTools.map(tool => (
                                ...
                                >
                                    ...
                                    <span className="text-[10px] font-black opacity-10 group-hover:opacity-100 group-hover:text-violet-500 transition-all">READY</span>
                                </motion.div>
                            ))}
                        </div>
                    </div>
                ...
                {/* 2. FLAGSHIP MODULES (Right Main Area) */}
                <div className="lg:col-span-8 space-y-10">
                    
                    {/* SOCIAL ARENA FLAGSHIP */}
                    <div className="relative group cursor-pointer" onClick={() => navigate('/group-quiz')}>
                         ...
                         <Card variant="dark" className="relative !p-16 !rounded-[4rem] border-white/10 bg-[#020408] overflow-hidden flex flex-col md:flex-row items-center gap-16">
                            <div className="absolute top-0 right-0 p-16 opacity-[0.03] group-hover:scale-125 transition-transform duration-1000"><UsersIcon className="w-96 h-96"/></div>
                            <div className="flex-grow space-y-8 text-center md:text-left">
                                <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-400">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest italic">Live Battles Active</span>
                                </div>
                                <h3 className="text-6xl md:text-9xl font-black text-white italic tracking-tightest uppercase leading-none">THE ARENA</h3>
                                <p className="text-xl md:text-2xl text-slate-400 font-medium italic leading-relaxed max-w-xl">Destroy your friends in real-time logic battles. Cross-grade matchmaking calibrated for 99th percentile dominance.</p>
                                <Button size="lg" className="h-20 px-16 !text-xl !font-black !rounded-full !bg-white !text-black shadow-2xl hover:scale-105 transition-all italic">ENTER ARENA &rarr;</Button>
                            </div>
                            <div className="hidden md:flex flex-col gap-4">
                                {[1,2,3].map(i => (
                                    <div key={i} className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center opacity-40 group-hover:opacity-100 transition-all duration-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <Card variant="dark" className="!p-12 !rounded-[4rem] border-white/5 hover:border-pink-500/30 transition-all cursor-pointer bg-slate-900/30" onClick={() => navigate('/study-planner')}>
                             <h4 className="text-[10px] font-black text-pink-500 uppercase tracking-[0.5em] mb-4 italic">Strategic Astra</h4>
                             <div className="flex items-center gap-6 mb-8">
                                <div className="p-4 bg-pink-500/10 rounded-2xl border border-pink-500/20 text-pink-500"><CalendarDaysIcon className="w-10 h-10"/></div>
                                <h3 className="text-4xl font-black text-white italic tracking-tighter uppercase leading-none">WAR ROOM</h3>
                             </div>
                             <p className="text-slate-500 text-base font-medium italic leading-relaxed mb-10">Constraint-based tactical study scheduling. Peak focus optimization.</p>
                             <div className="h-1.5 w-1/2 bg-pink-500/20 rounded-full overflow-hidden"><div className="h-full w-2/3 bg-pink-500"></div></div>
                        </Card>

                        <Card variant="dark" className="!p-12 !rounded-[4rem] border-white/5 hover:border-violet-500/30 transition-all cursor-pointer bg-slate-900/30" onClick={() => handleToolClick('quiz', true)}>
                             <h4 className="text-[10px] font-black text-violet-500 uppercase tracking-[0.5em] mb-4 italic">Simulated Combat</h4>
                             <div className="flex items-center gap-6 mb-8">
                                <div className="p-4 bg-violet-500/10 rounded-2xl border border-violet-500/20 text-violet-500"><QuestIcon className="w-10 h-10"/></div>
                                <h3 className="text-4xl font-black text-white italic tracking-tighter uppercase leading-none">GAMEVERSE</h3>
                             </div>
                             <p className="text-slate-500 text-base font-medium italic leading-relaxed mb-10">Play your chapter as a high-fidelity 3D mission. Knowledge is your XP.</p>
                             <div className="h-1.5 w-1/2 bg-violet-500/20 rounded-full overflow-hidden"><div className="h-full w-1/3 bg-violet-500"></div></div>
                        </Card>
                    </div>
                </div>
            </div>
            ...
            <AnimatePresence>
                {showSyncModal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-4xl flex items-center justify-center p-8">
                         <Card variant="dark" className="max-w-3xl !p-20 !rounded-[5rem] border-violet-500/20 text-center space-y-12 relative overflow-hidden">
                             ...
                             <p className="text-2xl text-slate-400 font-medium italic leading-relaxed">This module requires chapter data to operate. Synergize your notes or curriculum content to proceed.</p>
                             <div className="flex flex-col md:flex-row gap-6 justify-center">
                                 <Link to="/new-session" className="flex-grow"><Button size="lg" className="w-full h-24 !text-2xl !font-black !rounded-full !bg-white !text-black shadow-2xl">SYNC CONTENT NOW &rarr;</Button></Link>
